...
export class AnalyticsService {
  ...
  async trackEvent(action: string, subject: string, userName: string, namespace: string): Promise<void> {
    try {
      ...
      let analytics = await this.knex('analytics')
        .where({ subject })
        .first();

      if (!analytics) {
        const [newAnalytics] = await this.knex('analytics')
          .insert({
            action,
            subject,
            access_count: 1,
      ...
      const [updatedAnalytics] = await this.knex('analytics')
      ...
      let userMetrics = await this.knex('analytics_metrics_users')
        .where({
      ...
      if (!userMetrics) {
        await this.knex('analytics_metrics_users').insert({
          analytics_id: updatedAnalytics.id,
          name: userName,
          namespace: namespace
        });
      } else {
        await this.knex('analytics_metrics_users')
  ...
  async getAnalytics(subject?: string): Promise<AnalyticsWithMetrics[]> {
    const query = this.knex('analytics')
      .select(
        'analytics.*',
  ...
  async getAnalyticsById(id: string): Promise<AnalyticsWithMetrics | null> {
    const subquery = this.knex('analytics_metrics_users')
      .select(
        this.knex.raw(
          `json_agg(
            json_build_object(
    ...
    const result = await this.knex('analytics')
      .select(
        'analytics.*',
        subquery
      )
  ...
  async getTopUsers(): Promise<{ name: string; access_count: number }[]> {
    try {
      const results = await this.knex('analytics_metrics_users')
        .select('name')
