...
import { testebackPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [testebackPlugin],
    });

    await request(server).get('/api/testeback/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/testeback/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/testeback/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/testeback/todos/${createdTodoItem.id}`)
